...
This guide aims to provide a detailed walkthrough of how data can be published according to the FAIR principles. It builds on [`washr`](https://github.com/openwashdata/washr), an R package developed for swift data publication. The package emerged from the need to streamline certain steps when publishing the datasets collected during [GHE's openwashdata academy](https://openwashdata.org/pages/academy/). The code in this guide was last checked against washr version `r readLines("WASHR_VERSION", warn = FALSE)[1]`, the version recorded in the `WASHR_VERSION` file of the [source repository](https://github.com/Global-Health-Engineering/ghedatapublishing).
