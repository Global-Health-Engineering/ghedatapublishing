...
Choose a name for your repository, make it public if your data is not confidential/sensitive and click on "Create repository". For now, you don't need a README, .gitignore file or a license. You'll add these at a later stage.
...
Before you can connect your project to GitHub, your project needs to be a Git repository with at least one commit. Run the following command in the Console:

```{r}
usethis::use_git()
```

When asked whether it is ok to commit the files, accept the initial commit. Afterwards, `usethis` asks to restart RStudio so that the Git tab appears. Accept this as well and wait for RStudio to restart.
...
If everything went well, your local repository (on your computer) should now be connected to the remote repository (on GitHub) which allows you to push and pull changes and to invite collaborators.
